...
## 1.10
1. 체크박스도 어찌어찌 구현해야됨 -> 진척도와 연결

## 1.11
1. 월간 새 일정 클릭시 키보드 치는거 안보임 -> 바로 수정화면 띄우던가 개선 필요

## 1.13 할일 정리
1. 주간 보기 화면 아래 공간 채우기 -> 진척도와 관련된 이모티콘으로 채움
- Drawer 에 설정 구현
1. 월요일 시작 / 일요일 시작 설정 추가
2. 컬러 테마
3. 다른 날짜 누르면 textfield 비우기 및 unfocus
4. bottomsheet에서 textformfield 누르면 커서 위치 이상한거 고치기
5. 새일정 추가의 textformfield에서 편집버튼 누르면 치다가 만 text 전달해주기
6. 월간화면에서는 누르면 바로 수정 화면 올라오도록